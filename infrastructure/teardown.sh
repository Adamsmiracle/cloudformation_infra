#!/usr/bin/env bash
# =============================================================================
# Tear down the ENTIRE photo-uploader infrastructure (for recreate-before-review).
#
#   Usage:
...
#     1. Empties every photo-uploader-* S3 bucket (all object versions + delete
#        markers) and the ECR repo — these block CloudFormation stack deletion.
#     2. Deletes the parent stack  (default: "photo-uploader")   -> all nested stacks
#        (incl. the CloudFront distribution, which takes ~15-20 min to delete)
#     3. Deletes the bootstrap stack (default: "photo-uploader-bootstrap")
...
delete_stack() {
  ...
  if aws cloudformation describe-stacks --stack-name "$s" --region "$REGION" >/dev/null 2>&1; then
    echo "Deleting stack: $s (this can take 15-20 min — CloudFront/RDS are slow)..."
...
echo -n "CloudFront   : "; aws cloudfront list-distributions --query "DistributionList.Items[?contains(Comment,'${PROJECT}')].Id" --output text 2>/dev/null | grep . || echo "none ✅"
